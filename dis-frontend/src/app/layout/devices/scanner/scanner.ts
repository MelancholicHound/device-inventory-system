...
export class DeviceScanner {
  ...
  scannerType = signal<any>([]);

  scannerForm!: FormGroup;

  constructor() {
    this.scannerForm = this.createScannerForm();

    this.requestAuth.getAllScannerBrand().subscribe((res: any) => this.brand.set(res));
    this.requestAuth.getScannerTypes().subscribe((res: any) => this.scannerType.set(res));
  ...
  postScanner(): void {
    const rawValue: any = this.scannerForm.value;
    ...
    this.requestAuth.postScanner(duplicatedArray).subscribe({
      next: (res: any) => {
